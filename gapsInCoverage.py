...
class gapsInCoverage(IonPlugin):
    ...
    def runsambamba(self, results_dir, bedfile, barcode, bam, max_cov=None, prefix=None):
        ...
        if prefix:
            ...
            self.run_command('/home/ionadmin/sambamba_v0.6.5 depth base -o ' + outputfile_filtered + ' -c 0 -C ' + str(max_cov) + ' -q 0 -L ' + bedfile + ' ' + bam)
    ...
    def write_to_excel(self, outfile, alamut_file, cov_file, num_gaps, min_cov):
    ...
    def launch(self, data=None):
        ...
        for bam in bams:
            ...
            depth_base_file = self.runsambamba(results_dir, os.path.join(results_dir, (bed_name+'_noheader.bed')), barcode, bam)
            exonic_gaps = self.runsambamba(results_dir, os.path.join(results_dir, (bed_name+'_exonic_noheader.bed')), barcode, bam, 49, '_exonic')
            intronic_gaps = self.runsambamba(results_dir, os.path.join(results_dir, (bed_name+'_intronic_noheader.bed')), barcode, bam, 29, '_intronic')
            ...
            excel_file = self.write_to_excel(os.path.join('/results/for_review', run_name, barcode + '_' + info[barcode]['sample'] + '.xls'), alamut_file, depth_base_file, total_gaps, min_cov)
