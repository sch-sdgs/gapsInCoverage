...
def runsambamba(results_dir, bedfile, barcode, bam, max_cov=None, prefix=None):
    ...
    if prefix:
        ...
        run_command(os.environ['DIRNAME']+'/bin/sambamba_v0.6.5 depth base -o ' + outputfile + ' -m -c 0 -q 0 -L ' + bedfile + ' ' + bam)
        outputfile_filtered = os.path.join(results_dir, (barcode + prefix + '_depth_base_filtered.txt'))
        run_command(os.environ['DIRNAME']+'/bin/sambamba_v0.6.5 depth base -o ' + outputfile_filtered + ' -m -c 0 -C ' + str(max_cov) + ' -q 0 -L ' + bedfile + ' ' + bam)
...
def start(jsonfile):
    ...
    for sample_name in data["plan"]["barcodedSamples"]:
        for barcode in data["plan"]["barcodedSamples"][sample_name]["barcodeSampleInfo"]:
            ...
            if os.path.isfile(os.path.join(results_dir, (bed_name + '_noheader.bed'))):
                ...
                run_command(os.environ['DIRNAME']+'/bin/subtractBed -a '+ os.path.join(results_dir, (bed_name + '_noheader.bed')) + ' -b ' + os.path.join(results_dir, (bed_name + '_exonic_noheader.bed')) + ' > ' + os.path.join(results_dir, (bed_name + '_intronic_noheader.bed')))
    ...
    for bam in bams:
        ...
        run_command(os.environ['DIRNAME']+'/bin/sambamba_v0.6.5 depth region -o ' + os.path.join(results_dir, (barcode + '_exonic_depth_region.txt')) + ' -c 0 -T 50 -q 0 -L ' + bed_name+'_exonic_noheader.bed ' + bam)
        cov_hash={}
        ...
        run_command((os.environ['DIRNAME']+'/bin/intersectBed -wb -a ' + os.path.join(results_dir, (bed_name+'_noheader.bed')) + ' -b ' + gaps_bed_file + ' | cut -f1,2,4,12 | awk \'BEGIN{print "chromosome\tbp_pos\tregion\tdepth"}1\' > ' + gaps_in_sequencing))
